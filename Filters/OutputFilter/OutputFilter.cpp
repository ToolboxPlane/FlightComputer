    ...
    void OutputFilter::run() {
        ...
        while (!in.isClosed()) {
            if (in.get(control)) {
                ...
                serialOutPkg.setChannel(3, static_cast<uint16_t>(angleToStart * 2.0F + 500.0F * si::scalar));
                ...
                baseOutPkg.setChannel(2, static_cast<uint16_t>(angleToStart * 2.0F + 500.0F * si::scalar));
